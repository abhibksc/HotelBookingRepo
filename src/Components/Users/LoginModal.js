...
const LoginModal = () => {
    ...
    if (MyInput) {
      ...
      if (otpDetails && otpDetails.otp && otpDetails.otp_id) {
        ...
        if (response && response.message === "Login successful.") {
          ...
          response &&
          response.error &&
          response.error.otp.length > 0 &&
          response.error.otp[0] === "The selected otp is invalid."
        ) {
          return toast.error("Invalid Otp");
        } else {
  ...
  return (
    <>
      {
        <div className="fixed inset-0 flex items-center justify-center bg-white/5 backdrop-blur-md z-50 transition-opacity duration-300 ease-out opacity-100">
                  ...
                  <div className="px-6 flex flex-col gap-6 text-center">
                    ...
                    {otpDetails && otpDetails.otp_id && (
                      <div className="flex flex-row gap-3">
                        ...
                        <button
                          type="button"
                          className="border p-2 rounded-md bg-yellow-600 text-black font-medium hover:bg-yellow-600 transition-all mx-auto"
