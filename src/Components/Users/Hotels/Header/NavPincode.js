...
const NavPincode = () => {
  ...
  // setLoading
  const email = useSelector((state) => state.auth.roles.user.email);
  const AreaPin = useSelector((state) => state.auth.roles.user.AreaPin);
    ...
    try {
      ...
      if (data[0].Status === "Success") {
        ...
        if (
          ...
          AvailabilityResponse.data.message === "Available"
        ) {
          ...
          AvailabilityResponse &&
          AvailabilityResponse.data.message === "Not Available"
        ) {
          triggerVibration();
          setIsValid(false);
          setPincodeInfo(null);
          setPincode(AreaPin);
          toast.warn(
            "Sorry, delivery services are unavailable in your location."
          );
        } else {
          triggerVibration();
          setIsValid(false);
